...
import {
  LayoutDashboard, Settings, LogOut, Briefcase, Award, Gift, 
  Menu, X, Building2
} from 'lucide-react';
...
const menuItems = [
  ...
  { label: 'Leaderboard', icon: Award, path: '/employee/leaderboard' },
  { label: 'Rewards', icon: Gift, path: '/employee/rewards' },
...
export const EmployeeLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const getUserInfo = () => {
    ...
    return {
      name,
      roleName: user?.role_name || 'Employee Referrer',
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0f]">
      {/* Animated Background - Orange/Amber Theme for Employee */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <motion.div
          className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-amber-500/10 rounded-full blur-[150px]"
          animate={{
            scale: [1, 1.2, 1],
        ...
        />
        <motion.div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-yellow-500/5 rounded-full blur-[180px]"
          animate={{
            rotate: [0, 360],
          }}
            ...
            <AnimatePresence>
              {isExpanded && (
                ...
                >
                  ...
                  <span className="text-xs text-white/50 font-medium whitespace-nowrap">Employee Portal</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'bg-gradient-to-r from-amber-500/20 to-amber-500/5 text-white'
                      : 'text-white/60 hover:bg-white/5 hover:text-white'
                  )}
                >
                  {isActive && (
                    <motion.div
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-amber-500 rounded-full shadow-lg shadow-amber-500/50"
                  ...
                  {!isActive && (
                    <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-amber-500/10 to-transparent pointer-events-none" />
                  )}
                </Link>
              </motion.div>
            );
        ...
        <div className="p-4 border-t border-white/5">
          ...
          >
            <div className="relative flex-shrink-0">
              <motion.div
                className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center text-white font-bold shadow-lg"
                whileHover={{ rotate: 5 }}
              >
                {userInfo.initials}
              </motion.div>
                    ...
                    >
                      ...
                      >
                        <div className={cn(
                          ...
                          isActive ? 'bg-amber-500 text-white' : 'bg-white/5'
                        )}>
