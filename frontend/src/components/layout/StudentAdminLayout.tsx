...
export const StudentAdminLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const getUserInfo = () => {
    const name = user?.full_name || 'Student Admin';
    return {
      name,
      role: 'Student Admin',
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0f]">
      ...
      >
        ...
        <div className="h-20 flex items-center px-5 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-400 flex items-center justify-center shadow-lg shadow-emerald-500/30">
            ...
            <AnimatePresence>
              {isExpanded && (
                ...
                >
                  ...
                  <span className="text-xs text-emerald-400 font-medium whitespace-nowrap">Student Admin Portal</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        <nav className="flex-1 py-6 px-3 space-y-1.5 overflow-y-auto overflow-x-hidden">
          ...
          {menuItems.map((item, index) => {
            const isActive = location.pathname === item.path || 
              (item.path !== '/student-admin' && location.pathname.startsWith(item.path));
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'bg-gradient-to-r from-emerald-500/20 to-emerald-500/5 text-white'
                ...
                >
                  ...
                  {isActive && (
                    <motion.div 
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-emerald-500 rounded-full shadow-lg shadow-emerald-500/50"
                      layoutId="studentAdminActiveIndicator"
                  ...
                  {/* Hover glow effect */}
                  {!isActive && (
                    <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-emerald-500/10 to-transparent pointer-events-none" />
                  )}
                </Link>
              </motion.div>
            );
        ...
        <div className="p-4 border-t border-white/5">
          ...
          >
            <div className="relative flex-shrink-0">
              <motion.div 
                className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-400 flex items-center justify-center text-white font-bold shadow-lg"
                whileHover={{ rotate: 5 }}
              >
                {userInfo.initials}
              </motion.div>
              ...
              {isExpanded && (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-emerald-400 truncate">{userInfo.role}</p>
                  </div>
                  <motion.button 
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-bold text-lg text-white">Student Admin</span>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
                    ...
                    >
                      ...
                      >
                        <div className={cn(
                          ...
                          isActive ? 'bg-emerald-500 text-white' : 'bg-white/5'
                        )}>
              ...
              <div className="mt-8 p-4 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-400 flex items-center justify-center text-white font-bold">
                    {userInfo.initials}
                  </div>
                  <div className="flex-1">
                    <p className="font-semibold text-white">{userInfo.name}</p>
                    <p className="text-sm text-emerald-400">{userInfo.role}</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
      ...
      <main className="flex-1 overflow-auto relative z-10">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="font-bold text-lg text-white">Student Admin</h1>
                    <p className="text-xs text-emerald-400">Management Portal</p>
                  </div>
                </div>
              </div>
        ...
        <div className="p-6 lg:p-8 max-w-[1800px] mx-auto text-white">
          {children}
        </div>
      </main>
    </div>
