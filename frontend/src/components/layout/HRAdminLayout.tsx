...
import {
  LayoutDashboard, Settings, LogOut, Award, Users, 
  Menu, X, Shield, MessageSquare, Cog
} from 'lucide-react';
...
const menuItems = [
  { label: 'Dashboard', icon: LayoutDashboard, path: '/hr-admin' },
  { label: 'Leaderboard', icon: Award, path: '/hr-admin/leaderboard' },
  { label: 'Operations', icon: Cog, path: '/hr-admin/operations' },
  { label: 'Request Centre', icon: MessageSquare, path: '/hr-admin/requests' },
  { label: 'Settings', icon: Settings, path: '/hr-admin/settings' },
];

export const HRAdminLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const getUserInfo = () => {
    const name = user?.full_name || 'HR Admin';
    return {
      name,
      roleName: user?.role_name || 'Human Resources',
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0f]">
      {/* Animated Background - Blue/Indigo Theme for HR Admin */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <motion.div
          className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-blue-500/10 rounded-full blur-[150px]"
          animate={{
            scale: [1, 1.2, 1],
        ...
        />
        <motion.div
          className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-indigo-500/10 rounded-full blur-[150px]"
          animate={{
            scale: [1, 1.3, 1],
            opacity: [0.1, 0.2, 0.1]
          }}
        ...
        />
        <motion.div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-purple-500/5 rounded-full blur-[180px]"
          animate={{
            rotate: [0, 360],
          }}
            ...
            <AnimatePresence>
              {isExpanded && (
                ...
                >
                  ...
                  <span className="text-xs text-white/50 font-medium whitespace-nowrap">HR Admin Portal</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        <nav className="flex-1 py-6 px-3 space-y-1.5 overflow-y-auto overflow-x-hidden">
          ...
          {menuItems.map((item, index) => {
            const isActive = location.pathname === item.path ||
              (item.path !== '/hr-admin' && location.pathname.startsWith(item.path));
            ...
            return (
              ...
              >
                ...
                >
                  {isActive && (
                    <motion.div
                      className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-blue-500 rounded-full shadow-lg shadow-blue-500/50"
                  ...
                  {!isActive && (
                    <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-blue-500/10 to-transparent pointer-events-none" />
                  )}
                </Link>
              </motion.div>
            );
        ...
        <div className="p-4 border-t border-white/5">
          ...
          >
            <div className="relative flex-shrink-0">
              <motion.div
                className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold shadow-lg"
                whileHover={{ rotate: 5 }}
              >
                {userInfo.initials}
              </motion.div>
              ...
              {isExpanded && (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/50 truncate">{userInfo.roleName}</p>
                  </div>
                  <motion.button
              ...
              <div className="mt-8 p-4 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-white/50">{userInfo.roleName}</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/50">HR Admin Portal</p>
                  </div>
                </div>
              </div>
